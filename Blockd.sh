...
# Configure menu added, choose the number of reset packets you want to send, own blocklist, temporary file cleaning
...
####issue, tells anyway that the file already exists
function create_own_blocklist {
file2=own_blocklist.blockdd
echo ""
echo "Enter the sites that you want to block"
echo "Put each of them into new line"
if [ -e $file ]; then
echo "You already have an own blocklist. Opening for editing."
sudo nano $file2
else
echo "You have not yet created you own blocklist"
echo "Creating now..."
sudo nano $file2
fi
}
...
echo | awk -vORS='|' 5 own_blocklist.blockdd > blocklist.blockdd
path2=`sed '$s/.$//' blocklist.blockdd`
echo ""
...
function packets {
echo ""
echo "How many reset packets you want to send?"
...
PS3='
Select an option:'
select option in "Block Facebook" "Block Google" "Block Facebook & Google" "Manual Block" "Own blocklist" "Configure" "Allow all" "Quit"
